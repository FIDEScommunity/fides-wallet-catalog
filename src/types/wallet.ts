...
import type {
  WalletDeploymentModel,
  WalletLicense,
} from '../constants/wallet-v2-limits';

export type { WalletDeploymentModel, WalletLicense } from '../constants/wallet-v2-limits';
...
/** Crawl-time overlay from iGrant EUDI wallet tracker (not in community JSON schema). */
...
export interface EudiTrackerEnrichment {
  ...
  badgeSvgUrl?: string;
  status: string;
  /** Index into iGrant statusOrder (lower = further along). */
...
export interface NormalizedWallet extends Wallet {
  ...
  /** Present only in aggregated.json when matched to iGrant tracker. */

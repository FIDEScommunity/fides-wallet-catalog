(function() {
  ...
  const icons = {
    ...
    award: '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="8" r="6"/><path d="M15.477 12.89 17 22l-5-3-5 3 1.523-9.11"/></svg>',
  ...
  function buildModalAwardRecognitionBadgeHtml(recognition) {
    if (!recognition || typeof recognition !== 'object') return '';
    const winner = Number(recognition.place) === 1;
    const result = winner ? 'Winner' : 'Finalist';
    const year = Number(recognition.year) || null;
    const label = year ? result + ' ' + year : result;
    const category = recognition.categoryLabel ? String(recognition.categoryLabel) : '';
    const program = recognition.programLabel ? String(recognition.programLabel) : '';
    const title = [label, category, program].filter(Boolean).join(' · ');
    const content = icons.award + '<span>' + escapeHtml(label) + '</span>';
    const className = 'fides-modal-award-inline award-' + (winner ? 'winner' : 'finalist');
    const url = recognition.programUrl ? String(recognition.programUrl).trim() : '';
    if (url) {
      return ' <a class="' + className + '" href="' + escapeHtml(url) +
        '" target="_blank" rel="noopener noreferrer" title="' + escapeHtml(title) +
        '" onclick="event.stopPropagation();">' + content + '</a>';
    }
    return ' <span class="' + className + '" title="' + escapeHtml(title) + '">' + content + '</span>';
  }
  ...
  function openWalletModal(wallet, options) {
    ...
    const awardRecognitionHtml = buildModalAwardRecognitionBadgeHtml(options && options.awardRecognition);
    ...
    const modalHtml = '<div class="fides-modal-overlay fides-modal-overlay--rp fides-modal-overlay--wallet" id="fides-modal-overlay" data-theme="' + escapeHtml(theme) + '">' +
      ...
      '<div class="fides-modal-title-wrap"><div class="fides-modal-title-row"><h2 class="fides-modal-title">' + escapeHtml(wallet.name) + '</h2>' + listingHeaderBadge + '</div><p class="fides-modal-provider">' + icons.building + ' ' + providerNameInHeader + countryModalFlagHtml + awardRecognitionHtml + buildModalBluePagesLinkHtml(bluePagesUrl) + '</p></div>' +
  ...
  window.FidesCatalogUI = {
    ...
    buildModalAwardRecognitionBadgeHtml,
